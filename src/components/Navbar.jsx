...
const NAV_LINKS = [
  ...
  { to: '/suggest', label: 'Suggest',  icon: '🗣️' },
]
  ...
  return (
    <>
      ...
      <header style={{
        ...
        <div className='home-btn'>
          <button 
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', gap: 10, marginRight: 6, flexShrink: 0,
              padding: '4px 0',
            }}
          >
            {/* Icon mark */}
            <div style={{
              ...
              boxShadow: '0 0 16px rgba(0,212,255,.3)',
            }}>🎮</div>
            {/* Wordmark */}
            <span style={{
        ...
        {/* Right side */}
        <div style={{ display: 'flex', alignItems: 'right', gap: 8, flexShrink: 0 }}>
          {user ? (
            /* User menu */
            <div style={{ position: 'relative' }} className="user-btn">
              <button
                ...
                style={{
                  background: showUserMenu ? 'rgba(0,212,255,.08)' : 'none',
                  border: `1px solid ${showUserMenu ? 'rgba(0,212,255,.25)' : 'var(--border)'}`,
              ...
              >
                <Avatar user={user} profile={profile} size={28} /><h4 style={{color: 'white'}}>{user.displayName}</h4> 
              ...
              {showUserMenu && (
                <>
                  ...
                  <div className="anim-slide-down" style={{
                    ...
                    borderRadius: 12, padding: '8px 6px', minWidth: 200,
                    boxShadow: 'var(--shadow-xl), 0 0 30px rgba(0,212,255,.08)',
                    zIndex: 11,
                  }}>
                    {/* User info */}
                    <div style={{ padding: '10px 12px 12px', borderBottom: '1px solid var(--border)', marginBottom: 6 }}>
                      <p style={{ fontSize: '.85rem', fontWeight: 700, color: 'var(--text0)', fontFamily: 'var(--font-body)' }}>
                        {profile?.displayName || user.displayName}
                      </p>
                      <p style={{ fontSize: '.72rem', color: 'var(--text3)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {user.email}
                      </p>
                    </div>
                    ...
                    {[
                      ...
                      <button key={path} onClick={() => go(path)}
                        style={{
                          ...
                          color: 'var(--text1)', padding: '9px 12px', textAlign: 'left',
                          borderRadius: 7, fontSize: '.83rem', cursor: 'pointer',
                    ...
                    <div style={{ borderTop: '1px solid var(--border)', marginTop: 6, paddingTop: 6 }}>
                      <button onClick={handleLogOut}
                        style={{
                          width: '100%', background: 'none', border: 'none',
                          color: 'var(--red)', padding: '9px 12px', textAlign: 'left',
                          borderRadius: 7, fontSize: '.83rem', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', gap: 9,
            ...
            <div className="desktop-auth" style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-ghost" style={{ padding: '7px 14px', fontSize: '.82rem' }}
                onClick={() => openAuth('login')}>Sign In</button>
              <button className="btn btn-primary" style={{ padding: '7px 14px', fontSize: '.82rem' }}
                onClick={() => openAuth('signup')}>Join Free</button>
            </div>
          )}
          ...
          <button
            ...
            style={{
              display: 'none',
              background: showMobile ? 'rgba(0,212,255,.1)' : 'var(--bg3)',
              border: `1px solid ${showMobile ? 'rgba(0,212,255,.3)' : 'var(--border2)'}`,
              color: showMobile ? 'var(--cyan)' : 'var(--text1)',
      ...
      {showMobile && (
        <>
          <div
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 148, background: 'rgba(7,8,13,.75)', backdropFilter: 'blur(4px)' }}
          />
          <div className="anim-slide-down" style={{
            position: 'fixed', top: 'var(--nav-h)', left: 0, right: 0, zIndex: 149,
              ...
              {NAV_LINKS.map(({ to, label, icon }) => (
                <NavLink
                  ...
                  style={({ isActive }) => ({
                    ...
                    fontWeight: isActive ? 700 : 500, fontSize: '.95rem',
                    color: isActive ? 'var(--cyan)' : 'var(--text1)',
                    background: isActive ? 'rgba(0,212,255,.07)' : 'none',
                    border: `1px solid ${isActive ? 'rgba(0,212,255,.2)' : 'transparent'}`,
                    textDecoration: 'none',
                  })}
                >
                  ...
                  <span>{label}</span>
                  {to === '/library' && libraryCount > 0 && (
                    <span style={{ background: 'var(--grad)', color: '#fff', borderRadius: 999, minWidth: 20, height: 20, fontSize: '.68rem', fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 6px', marginLeft: 'auto' }}>
                      {libraryCount}
                    </span>
                  )}
                </NavLink>
            ...
            {user ? (
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 3 }}>
                <button onClick={() => go(`/profile/${user.uid}`)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 9, background: 'none', border: 'none', color: 'var(--text1)', fontSize: '.92rem', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
